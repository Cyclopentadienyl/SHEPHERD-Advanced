...
│                         ↓                                                    │
│  【第二站：病歷資料庫】                                                       │
│   查詢醫學知識庫，找出症狀、基因、疾病之間的關聯                              │
│                         ↓                                                    │
│  【第三站：專家會診室】                                                       │
│   沿著「症狀 → 基因 → 疾病」的路徑推理，找出最可能的診斷                      │
│                         ↓                                                    │
│  【第四站：AI 助理（可選）】                                                  │
│   用深度學習模型對候選疾病重新評分，提高準確度                                │
│                         ↓                                                    │
│  【第五站：報告撰寫】                                                         │
│   生成人類可讀的診斷報告，說明推理依據                                        │
│                                                                              │
...
以下用「醫院各部門」的比喻，說明系統中每個模組扮演的角色：

| 模組名稱 | 比喻角色 | 做什麼事？ | 為什麼需要它？ |
|---------|---------|-----------|---------------|
| **InputValidator**<br>（輸入驗證器） | 掛號櫃檯人員 | 檢查患者資料是否完整、症狀代碼格式是否正確（如 HP:0001250） | 確保後續流程收到的資料品質良好，避免「垃圾進、垃圾出」 |
| **KnowledgeGraph**<br>（知識圖譜） | 醫學百科全書 | 儲存「症狀↔基因↔疾病」的關聯網路，以及它們之間的連結強度 | 這是系統的**核心知識庫**，所有推理都基於這張「關係網」 |
| **PathReasoner**<br>（路徑推理器） | 資深診斷醫師 | 從患者的症狀出發，在知識圖譜中搜尋通往各種疾病的「推理路徑」 | 這是**核心推理引擎**——就像醫師在腦中思考「這個症狀可能是什麼基因問題？那個基因又會導致什麼疾病？」 |
| **ShepherdGNN**<br>（圖神經網路）| AI 第二意見 | 用深度學習技術，根據整體知識網路的結構，重新評估各疾病的可能性 | 補充傳統推理的不足，**提高診斷準確度**（類似請 AI 專家給第二意見） |
| **ExplanationGenerator**<br>（解釋生成器） | 報告撰寫員 | 把推理路徑轉換成人類可讀的文字說明，附上關鍵基因和文獻證據 | 確保診斷結果**可解釋、可追溯**，符合醫療 AI 的法規要求 |
| **VectorIndex**<br>（向量檢索庫） | 圖書館索引系統 | 快速找出「與患者症狀最相似」的疾病或基因候選 | 當知識庫包含數十萬筆資料時，**加速搜尋**，讓系統在毫秒內回應 |
...
─────────────────────────────────────────────────────────────────────────────
  ...
  └──────┬───────┘         ↓                ↓                  ↓
         │
         ▼            ┌─────────┐      ┌──────────┐      ┌─────────────┐
    標準化症狀代碼 ──→│確認格式 │──→  │找出相關  │──→  │搜尋推理路徑 │
    HP:0001250        │正確     │      │基因與疾病│      │計算分數     │
    HP:0001263        └─────────┘      └──────────┘      └──────┬──────┘
    HP:0000252                                                   │
                                                                 ▼
                                            ┌──────────────────────────────┐
