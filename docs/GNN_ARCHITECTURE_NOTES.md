...
## Cross-LLM 驗證結果（2026-05-14）

以下架構建議經過 Claude + Gemini 交叉驗證：

### 已驗證為真實且有價值

| 架構/技術 | 驗證狀態 | 來源 | 對我們的價值 |
|-----------|---------|------|-------------|
| **HL-HGAT** | ✅ 真實 | IEEE TPAMI 2026, PolyU, [GitHub](https://github.com/JH-415/HL-HGAT) | 高階拓撲（三角迴圈），捕捉「基因→表型→疾病→基因」閉環關係。實作複雜度高，benchmark 在腦影像非 KG |
| **HOPE** | ✅ 真實 | [arXiv:2601.05537](https://arxiv.org/abs/2601.05537) (2026-01) | Plug-and-play 解碼器，替換最後的線性層。在多個 HGNN backbone 上有提升，overhead 極小。**和我們的架構直接相容** |
| **SimpleHGN** | ✅ 真實 | KDD, [OpenHGNN](https://github.com/BUPT-GAMMA/OpenHGNN) (DGL) | 本質是 GAT + edge type embedding。和我們提的「GATv2 + type embedding」同一概念但有正式名字 |
| **SeHGNN 家族** | ✅ 真實 | AAAI 2023 + LMSPS (NeurIPS 2024) | 預計算鄰居聚合，訓練極快，VRAM 友好。已在前節詳述 |

### 已驗證為幻覺

| 聲明 | 驗證結果 | 真相 |
|------|---------|------|
| **Flash-HGT** | ❌ 不存在 | FlashAttention 存在，但沒有「Flash-HGT」這個專案。是通用技術被包裝成不存在的專有名詞 |
| **G-RMA** | ❌ 不存在 | Gradient checkpointing (`torch.utils.checkpoint`) 是真的，但「G-RMA」這個名字是捏造的 |

### GraphBolt — 採樣優化（真實但需跨框架）

Gemini 提出的反駁有技術道理：HGT 的 VRAM 問題不只是 float32，也因為子圖太大。GraphBolt（DGL 2.0+）對異質圖的 mini-batch 採樣做了底層優化，能讓 HGT 吃更小更精準的子圖。

**但 GraphBolt 是 DGL 框架的套件**，我們的系統建在 PyG 上。遷移框架工程成本大，不建議。

**PyG 對應方案**：`NeighborLoader` / `HGTLoader`，做類似的事：

#### NeighborLoader（通用異質圖採樣）
...
loader = NeighborLoader(
    ...
    batch_size=128,
    input_nodes=("disease", train_mask),
)
```

- 支援對每種邊類型分別設定採樣鄰居數
- 和 `HeteroConv`（GAT/GATv2/SAGE）直接相容
- 比我們自己寫的 `SubgraphSampler` 更高效
...
loader = HGTLoader(
    hetero_data,
    num_samples={key: [128] * 4 for key in hetero_data.node_types},
    batch_size=1024,
    input_nodes="disease",
)
```
...
- 專為 HGT 設計的採樣策略
- 維持各節點/邊類型的數量平衡
- 基於節點度數和已採樣連接數計算採樣概率，子圖更密集、資訊損失更小
- **可能是解決 HGT VRAM 問題的最佳 PyG 原生方案**

兩者都比我們目前的 `SubgraphSampler` 更成熟，值得在後續迭代中替換。

### 模組化拼裝共識

Encoder / Operator / Decoder 三層解耦的框架經兩個 LLM 獨立驗證：

```
Layer 1 — Encoder（互斥，選一個）：
  GAT / GATv2 / HGT / SeHGNN / HL-HGAT

Layer 2 — Operator（百搭，可疊加）：
  FlashAttention（需要 Encoder 有 attention）
  Gradient Checkpointing（通用）
  HGTLoader / NeighborLoader（採樣優化）

Layer 3 — Decoder（百搭，可替換）：
  Linear Head（目前使用）
  HOPE（plug-and-play 升級）
  Transformer Gene Encoder（PhenoKG 風格）
```
...
- [HL-HGAT (IEEE TPAMI 2026)](https://arxiv.org/abs/2403.06687)
- [HL-HGAT GitHub](https://github.com/JH-415/HL-HGAT)
- [HOPE (arXiv 2026)](https://arxiv.org/abs/2601.05537)
- [SimpleHGN via OpenHGNN (DGL)](https://github.com/BUPT-GAMMA/OpenHGNN)
- [PyG NeighborLoader Tutorial](https://pytorch-geometric.readthedocs.io/en/2.5.2/tutorial/neighbor_loader.html)
- [PyG HGTLoader Docs](https://pytorch-geometric.readthedocs.io/en/latest/modules/loader.html)
- [PyG Heterogeneous Graph Tutorial](https://pytorch-geometric.readthedocs.io/en/latest/tutorial/heterogeneous.html)
