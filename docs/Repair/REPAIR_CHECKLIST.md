...
- [x] **`train_model.py` ↔ `compute_shortest_paths.py` output mismatch** (RESOLVED by removal): `train_model.py:generate_synthetic_data()` wrote `node_features.pt`/`edge_indices.pt`/`num_nodes.json` and both sample files into a directory, with no `kg.json` and no `split_manifest.json`. Option (c) is now the only workflow: every workspace comes from `build_knowledge_graph.py --generate-samples`, which is what binds the graph export, the allocation and the cohorts into one production event. The function had no caller and no CLI flag; it was a second, unbound workspace producer, and a workspace it wrote is refused by the training preflight.
- [x] **`train_model.py` synthetic data has no biological meaning** (RESOLVED by removal): same function. `setup_demo.py` is the small-scale path.
