...
- [ ] **Systematic false positive — hyperprolinemia type 1**: This disease appeared in Top 10 across two unrelated test queries (Marfan + Rett phenotypes). GNN score is consistently high (~0.91) despite no clinical relevance. Investigate whether its node embedding has anomalous properties (e.g. high norm, generic direction) or if it's an artifact of insufficient training (3 epochs).
