...
| **5a** | **B-0.4 productionisation** — A becomes **the** shortest-path primitive rather than one of two: one builder over raw columns, one sort, the scan retired from the served path and kept only as an independent test reference. The plan it needs exists and is **revised and cleared to implement**; the blanket "no `src/inference/` edit" gate it carried is removed and the reason is recorded in its status block. Its *implementation* depends on no calibration, split or checkpoint decision; its **acceptance does need a designated loadable checkpoint** plus compatible graph and SP artifacts — see §3.5 | 5; acceptance also needs a loadable checkpoint | author + institution | **implemented, not accepted** — §10 steps 0-7 done and reviewed; step 8 half done: resident memory measured (plan §6.2, 9.00 bytes/row against the retired layout's 10.00), **peak at deployment scale and all of PLAN_B04 §13's integrated readings still pending a designated measurement subject**, and §13 reading 5 still blocked for want of the smallest supported target |
| **5b** | **SP artifact publication and binding** — the two defects `PLAN_B04_PRODUCTIONISATION.md` §8 left open: a tensor and a sidecar written with nothing recording that they belong together, and `--kg-path` / `--output-dir` independent so an artifact can land in a workspace it was not computed from. The consumer can now refuse a malformed table; neither check asks whether this is the *right* table. Plan: [`PLAN_SP_ARTIFACT_INTEGRITY.md`](PLAN_SP_ARTIFACT_INTEGRITY.md). **Needs no checkpoint and no institutional input**, which is why it runs ahead of 5a's remaining readings rather than behind them | — | author | **implemented** — producer, shared reader and all three consumers in one change; acceptance conditions tested and mutation-checked |
