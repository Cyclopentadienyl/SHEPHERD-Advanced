...
| D2 | **Closed by 1e.** `amp_dtype` and `torch_compiled` are on `MeasurementManifest`, and `torch_compiled` is on `DifferentialResult` too — that is the artifact whose regime actually varies. Two corrections to this row's own wording: `amp_enabled` was not "recorded", it was **hardcoded `False`** in `build_manifest`; and a structural claim is not an observation, so `assert_no_autocast` now **refuses** to run either traversal inside an autocast block rather than letting the manifest describe a run that did not happen. `observe_torch_compiled` is one function that never raises, not compile-metadata machinery |
