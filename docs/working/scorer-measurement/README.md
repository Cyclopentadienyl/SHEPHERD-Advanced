...
| B-0.4 | vectorised SP lookup | **both prototypes built and measured twice on the real artifact** (`PLAN_B04.md` §12). Approach A recommended, awaiting review: 8-33x faster on the caller production ships, 0 of 60 measurements over the provisional budget, at a cost of 3.44 GB steady-state. A measurement-order defect found after the first run was fixed and the run repeated; the repeat **reproduces every conclusion** (§12.7). **Independent of the calibration decision** — it consumes `shortest_paths.pt` and no checkpoint, split or model. Plan: [`PLAN_B04.md`](PLAN_B04.md) |
